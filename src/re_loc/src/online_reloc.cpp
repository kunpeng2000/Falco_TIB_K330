...
OnlineRelocalizationNode::OnlineRelocalizationNode(ros::NodeHandle& nh) 
    : nh_(nh), 
      relocalizer_(
          nh.param<double>("submap_radius", 10.0), 
          nh.param<double>("icp_thresh", 1.0), 
          nh.param<int>("icp_iters", 100),
          nh.param<double>("icp_score_thresh", 0.5)
      ),
    ...
    nh_.param<std::string>("base_frame", base_frame_, "camera_init");
...
bool OnlineRelocalizationNode::loadDatabaseAndMap() {
...
void OnlineRelocalizationNode::cloudCallback(const sensor_msgs::PointCloud2ConstPtr& msg) {
...
Eigen::Matrix4f OnlineRelocalizationNode::computeAveragePose(const std::vector<Eigen::Matrix4f>& poses) {
...
void OnlineRelocalizationNode::publishTfAndOdom(const Eigen::Matrix4f& T, const ros::Time& stamp) {
    // TF
...
void OnlineRelocalizationNode::mapTimerCallback(const ros::TimerEvent& event) {
